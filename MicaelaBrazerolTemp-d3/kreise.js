...
async function init() {
  ...
  const Xdisplacement = 80; //x achseverscheiben
  const circlesXcoordinate = 60; // displacement
  const circlesYcoordinate = 80;
  const dataValueScaling = 3; //kreis mal 3 für vergrösssern
  const hoehe = 200;
  ...
  circlesAnnual
    ...
    .attr('cy', (value, index) => {
      return value.annualMeanTemperatureFuture +100
    }) 
    .attr('r', (value, index) => {
      return value.annualMeanTemperatureFuture * dataValueScaling;
    })
  ...
  textLabel
    .append('text')
    .attr('x', (value, index) => {
      return index * Xdisplacement+ circlesXcoordinate - 20;
    })
    .attr('y', 300)
