...
async function init() {
  ...
  const padding = 0;
  const paddingBetweenDataMembers = 10;
  ...
  const Xdisplacement = 10; //x achseverscheiben
  ...
  const das = d3
    ...
    .append('div')
    .attr('id', 'das')
    .attr('class', 'das') // for styling in css
    .style('visibility', 'hidden')
    .text('a simple tooltip');

  const selection = svg.selectAll('rect').data(simplifiedData).enter();
  const textLabel = svg.selectAll('textLabel').data(simplifiedData).enter();
  ...
  selection
    ...
    .attr('x', (value, index) => {
      ...
      return index * spacePerDataMember + Xdisplacement;
    })
    .attr('y', (value, index) => {
  ...
  selection
    ...
    .attr('fill', '#ff5100') // Sets the color of the bars.
    .attr('x', (value, index) => {
      // Set the X position of the rectangle by taking the index of the current item we are creating, multiplying it by the calculated width of each bar, and adding a padding value so we can see some space between bars.
      return index * spacePerDataMember + barWidth + padding + Xdisplacement;
    })
    .attr('y', (value, index) => {
      // Set the rectangle by subtracting the scaled height from the height of the chart (this has to be done becuase SVG coordinates start with 0,0 at their top left corner).
  ...
  svg
    ...
    .on('mouseover', function () {
      ...
      // show tooltip
      d3.select('#das').text(city).style('visibility', 'visible');
    ...
    .on('mousemove', function (event) {
      ...
      d3.select('#das')
        ...
        .style('top', coords[3] - 10 + 'px')
        .style("color", 'black')
    })
    .on('mouseout', function () {
      d3.select(this).attr('fill', local.get(this));
      d3.select('#das').style('visibility', 'hidden');
    });
    ...
    .attr('y', 50)
    .attr('class', 'te')
    .text(`Temperature and Future Temperature of different Citys`)
